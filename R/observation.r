...
observation <- function(su.rast, obs, col.id, col.resp, method = "mls", ls.rast)
{
  ...
  obsLS <- terra::extract(su.rast,
                          obsSP,
                          method = "simple"
                        )
  obsSP[["SU"]] <- obsLS[, 2]

  # Data table from observations
  obsDT <- data.table::as.data.table(terra::values(obsSP))

  # Design based on maximum landscape similarity to SU (mls)
  if(method == "mls" ) {

    # Extract SU's landscape similarity values to each observation
    obsLS <- terra::extract(ls.rast,
                            obsSP,
                            method = "simple"
                          )

    # Merge SU's landscape similarity values with DT from observations
    obsDAT <- data.table::merge.data.table(obsDT,
                                           obsLS,
                                           by = base::colnames(obsDT)[1]
                                          )
    ...
    foreach::foreach(i = 4:base::ncol(obsDAT)) %do% {
      ...
      sucode <- stringi::stri_replace_all_regex(base::colnames(df)[4],
                                                "[a-zA-punct]",
                                                ""
                                              )
    ...
    ## Set of SU
    x <- base::unlist(base::as.list(terra::unique(su.rast)))
    ## Set of SU with representative observation
    y <- base::as.numeric(base::as.vector(repval$SU))
    ## Difference between sets
    norepval <- base::setdiff(x, y)

    # SpatVector of SUs' representative observations
    repval.sp <- terra::merge(obs,
                              repval[, c(1, 3, 4)],
                              by = base::colnames(repval)[1],
                              all.x = FALSE
                            )

    # Return objects
    base::list(su_repobs = repval,
    ...
    closestval <- function(x) x[base::order(base::abs(x - stats::median(x)))[1]]
    ...
    repval <- obsDT %>%
      dplyr::group_by(SU) %>%
        dplyr::mutate_at(dplyr::all_of(resp),
                         base::list(response = closestval)
                        )
    repval <- data.table::as.data.table(repval)
